...
class TransactionService(BaseService):
    ...
    async def get_by_user_paginated(
        ...
        *,
        ...
        db_transactions = await self.uow.repository.transactions.get_by_user_paginated(
            telegram_id,
            limit=limit,
            offset=offset,
        )
        logger.debug(
            f"Retrieved '{len(db_transactions)}' paginated transactions for user "
            f"'{telegram_id}' (limit={limit}, offset={offset})"
        )
        return TransactionDto.from_model_list(db_transactions)
    ...
    async def count_by_user(self, telegram_id: int) -> int:
        count = await self.uow.repository.transactions.count_by_user(telegram_id)
        logger.debug(f"Transactions count for user '{telegram_id}': '{count}'")
        return count
