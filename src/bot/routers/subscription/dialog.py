...
from src.core.enums import BannerName, CryptoAsset, DeviceType, PaymentGatewayType, PurchaseType
...
from .handlers import (
    ...
    on_duration_select,
    on_get_subscription,
    on_payment_asset_back,
    on_payment_asset_select,
...
payment_asset = Window(
    Banner(BannerName.SUBSCRIPTION),
    I18nFormat("msg-subscription-payment-asset"),
    Column(
        Select(
            text=Format("{item[label]}"),
            id=f"{PURCHASE_PREFIX}select_payment_asset",
            item_id_getter=lambda item: item["asset"],
            items="payment_assets",
            type_factory=CryptoAsset,
            on_click=on_payment_asset_select,
        ),
    ),
    Row(
        Button(
            text=I18nFormat("btn-back"),
            id=f"{PURCHASE_PREFIX}back_payment_asset",
            on_click=on_payment_asset_back,
        ),
    ),
    *back_main_menu_button,
    IgnoreUpdate(),
    state=Subscription.PAYMENT_ASSET,
    getter=payment_asset_getter,
)
...
confirm = Window(
    ...
    Row(
        Button(
            text=I18nFormat("btn-subscription-back-payment-asset"),
            id=f"{PURCHASE_PREFIX}back_payment_asset_confirm",
            on_click=on_subscription_confirm_back,
            when=F["show_payment_asset_back"],
        ),
        SwitchTo(
            text=I18nFormat("btn-subscription-back-payment-method"),
            id=f"{PURCHASE_PREFIX}back_payment_method",
            state=Subscription.PAYMENT_METHOD,
            when=~F["show_payment_asset_back"] & ~F["only_single_gateway"] & ~F["is_free"],
        ),
        SwitchTo(
            text=I18nFormat("btn-subscription-back-duration"),
            id=f"{PURCHASE_PREFIX}back_duration",
