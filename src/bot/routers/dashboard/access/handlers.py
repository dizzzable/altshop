...
async def on_condition_toggle(
    ...
    if callback.data == "rules":
        was_required = settings.rules_required
        new_state = not settings.rules_required
        settings.rules_required = new_state

        # Re-consent flow: when rules are enabled, all non-privileged users must accept again.
        if not was_required and new_state:
            await user_service.reset_rules_acceptance_for_non_privileged(accepted=False)
