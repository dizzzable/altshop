...
from src.infrastructure.taskiq.tasks.importer import (
    assign_plan_to_synced_users_task,
...
async def on_sync(
    ...
    active_plans = [plan for plan in await plan_service.get_all() if plan.is_active]
    has_active_plans = bool(active_plans)
    dialog_manager.dialog_data["sync_has_active_plans"] = has_active_plans
    ...
    warning_key = (
        "ntf-importer-sync-warning-has-active-plans"
        if has_active_plans
        else "ntf-importer-sync-warning-no-active-plans"
    )
    await notification_service.notify_user(user=user, payload=MessagePayload(i18n_key=warning_key))
...
async def on_assign_plan_select(
    ...
    dialog_manager.dialog_data["selected_assign_plan_id"] = selected_plan_id


@inject
async def on_assign_plan_all(
    callback: CallbackQuery,
    widget: Button,
    dialog_manager: DialogManager,
    notification_service: FromDishka[NotificationService],
    ...
    completed: dict = dialog_manager.dialog_data.get("completed", {})
    synced_telegram_ids: list[int] = completed.get("synced_telegram_ids", [])

    if not selected_plan_id:
        await notification_service.notify_user(
            user=user,
            payload=MessagePayload(i18n_key="ntf-importer-assign-plan-all-no-plan"),
        )
        return

    if not synced_telegram_ids:
        await notification_service.notify_user(
            user=user,
            payload=MessagePayload(i18n_key="ntf-importer-assign-plan-all-no-users"),
        )
        return

    if not is_double_click(dialog_manager, key="assign_plan_all_confirm", cooldown=10):
        await notification_service.notify_user(
            user=user,
    ...
    notification = await notification_service.notify_user(
        user=user,
        payload=MessagePayload.not_deleted(i18n_key="ntf-importer-assign-plan-all-started"),
    )

    task = await assign_plan_to_synced_users_task.kiq(selected_plan_id, synced_telegram_ids)
    result = await task.wait_result()
    ...
    await notification_service.notify_user(
        ...
        payload=MessagePayload(
            i18n_key="ntf-importer-assign-plan-all-done",
            i18n_kwargs=stats,
        ),
    )
