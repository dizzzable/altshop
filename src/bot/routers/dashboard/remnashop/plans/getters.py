...
async def configurator_getter(dialog_manager: DialogManager, **kwargs: Any) -> dict[str, Any]:
    ...
    helpers = {
        ...
        "is_unlimited_traffic": getattr(plan, 'is_unlimited_traffic', False),
    ...
    # Ensure subscription_count is always present
    data["subscription_count"] = getattr(plan, 'subscription_count', 1)
...
async def devices_getter(dialog_manager: DialogManager, **kwargs: Any) -> dict[str, Any]:
    adapter = DialogDataAdapter(dialog_manager)
    plan = adapter.load(PlanDto)
