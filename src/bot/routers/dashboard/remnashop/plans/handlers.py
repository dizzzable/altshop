...
@inject
async def on_subscription_count_input(
    message: Message,
    widget: MessageInput,
    dialog_manager: DialogManager,
    notification_service: FromDishka[NotificationService],
    ...
    dialog_manager.show_mode = ShowMode.EDIT
    user: UserDto = dialog_manager.middleware_data[USER_KEY]
    logger.debug(f"{log(user)} Attempted to set plan subscription count")

    if message.text is None or not (message.text.isdigit() and int(message.text) > 0):
        logger.warning(f"{log(user)} Invalid subscription count input: '{message.text}'")
        await notification_service.notify_user(
            user=user,
            payload=MessagePayload(i18n_key="ntf-plan-invalid-number"),
        )
    ...
    plan.subscription_count = number
    adapter.save(plan)

    logger.info(f"{log(user)} Successfully set plan subscription count to '{plan.subscription_count}'")
    await dialog_manager.switch_to(state=RemnashopPlans.CONFIGURATOR)
