...
user = Window(
    ...
    Row(
        Button(
            text=I18nFormat("btn-user-assign-plan"),
            id="assign_plan",
            on_click=on_assign_plan,
        ),
    ),
    Row(
        SwitchTo(
...
assign_plan = Window(
    ...
    IgnoreUpdate(),
    state=DashboardUser.ASSIGN_PLAN,
...
referrals = Window(
    Banner(BannerName.DASHBOARD),
    I18nFormat("msg-user-referrals", count=F["count"]),
    ScrollingGroup(
        Select(
...
purchase_discount = Window(
    ...
    IgnoreUpdate(),
    state=DashboardUser.PURCHASE_DISCOUNT,
    getter=purchase_discount_getter,
)
