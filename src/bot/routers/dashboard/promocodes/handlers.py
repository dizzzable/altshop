...
async def _append_allowed_user(
    *,
    dialog_manager: DialogManager,
    ...
    if target_telegram_id in promocode.allowed_user_ids:
        return False

    promocode.allowed_user_ids.append(target_telegram_id)
    adapter.save(promocode)
    dialog_manager.dialog_data.pop("allowed_search_users", None)
    return True
...
async def on_allowed_user_input(
    ...
    if not found_users:
        await message.answer(i18n.get("ntf-plan-no-user-found"))
        return

    if len(found_users) == 1:
        added = await _append_allowed_user(
            dialog_manager=dialog_manager,
            target_telegram_id=found_users[0].telegram_id,
        )
        if not added:
            await message.answer(i18n.get("ntf-plan-user-already-allowed"))
        return

    dialog_manager.dialog_data["allowed_search_users"] = [
        found_user.model_dump_json() for found_user in found_users
    ]
    await dialog_manager.switch_to(DashboardPromocodes.ALLOWED_USERS_RESULTS)


@inject
async def on_allowed_user_select(
    callback: CallbackQuery,
    widget: Select[int],
    dialog_manager: DialogManager,
    selected_user: int,
    ...
    added = await _append_allowed_user(
        dialog_manager=dialog_manager,
        target_telegram_id=selected_user,
    )
    if not added:
        await callback.answer(i18n.get("ntf-plan-user-already-allowed"), show_alert=True)
    await dialog_manager.switch_to(DashboardPromocodes.ALLOWED_USERS)
...
async def on_allowed_user_remove(
    ...
    if user_id in promocode.allowed_user_ids:
        promocode.allowed_user_ids.remove(user_id)
        adapter.save(promocode)


@inject
async def on_plan_filter_select(
    callback: CallbackQuery,
    widget: Select[int],
    dialog_manager: DialogManager,
...
@inject
async def on_clear_plan_filter(
    callback: CallbackQuery,
    widget: Button,
    dialog_manager: DialogManager,
) -> None:
...
async def on_confirm_promocode(
    ...
    if (
        promocode.availability == PromocodeAvailability.ALLOWED
        and len(promocode.allowed_user_ids) == 0
    ):
        await callback.answer(i18n.get("ntf-promocode-allowed-users-required"), show_alert=True)
        return
