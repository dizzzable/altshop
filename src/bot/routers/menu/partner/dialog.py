...
from .getters import (
    partner_balance_currency_getter,
...
from .handlers import (
    on_partner_balance_currency_select,
    on_withdraw,
    on_withdraw_all,
...
partner_main = Window(
    ...
    Row(
        SwitchTo(
            text=I18nFormat("btn-partner-balance-currency"),
            id="currency",
            state=UserPartner.CURRENCY,
        ),
        when=F["is_partner"],
    ),
    Row(
...
partner_currency = Window(
    Banner(BannerName.REFERRAL),
    I18nFormat(
        "msg-partner-balance-currency",
        current_currency=F["current_currency"],
        effective_currency=F["effective_currency"],
    ),
    Column(
        Select(
            text=I18nFormat(
                "btn-partner-balance-currency-choice",
                label=F["item"]["label"],
                selected=F["item"]["selected"],
            ),
            id="partner_balance_currency",
            item_id_getter=lambda item: item["id"],
            items="currency_list",
            type_factory=str,
            on_click=on_partner_balance_currency_select,
        ),
    ),
    Row(
        SwitchTo(
    ...
    IgnoreUpdate(),
    state=UserPartner.CURRENCY,
    getter=partner_balance_currency_getter,
)
