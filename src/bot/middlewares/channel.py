...
class ChannelMiddleware(EventTypedMiddleware):
    ...
    async def middleware_logic(
        ...
        chat_id, channel_link = self._resolve_channel_chat(settings)
        if chat_id is None:
            logger.warning(
        ...
        member = await self._get_chat_member_safe(
            bot=bot,
            user=user,
            chat_id=chat_id,
        ...
        await self._handle_unsubscribed_user(
            event=event,
            user=user,
    ...
    @staticmethod
    def _resolve_channel_chat(settings: Any) -> tuple[Union[str, int, None], str]:
        channel_link = settings.channel_link.get_secret_value()
        if settings.channel_has_username:
            return channel_link, channel_link
        if settings.channel_id:
            return settings.channel_id, channel_link
        return None, channel_link

    async def _get_chat_member_safe(
        self,
        bot: Bot,
        user: UserDto,
    ...
    async def _handle_unsubscribed_user(
        self,
        event: TelegramObject,
        user: UserDto,
        member_status: ChatMemberStatus,
