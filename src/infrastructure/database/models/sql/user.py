...
class User(BaseSql, TimestampMixin):
    ...
    partner_balance_currency_override: Mapped[Optional[Currency]] = mapped_column(
        Enum(
            Currency,
            name="currency",
            create_constraint=True,
    ...
    web_account: Mapped[Optional["WebAccount"]] = relationship(
        "WebAccount",
        foreign_keys="WebAccount.user_telegram_id",
        primaryjoin="User.telegram_id==WebAccount.user_telegram_id",
        uselist=False,
        lazy="selectin",
        back_populates="user",
    )
