...
class UserNotificationDto(TrackableDto):  # == UserNotificationType
    ...
    referral_qualified: bool = True
    partner_referral_registered: bool = True
    partner_earning: bool = True
    partner_withdrawal_request_created: bool = True
    partner_withdrawal_under_review: bool = True
    partner_withdrawal_completed: bool = True
    partner_withdrawal_rejected: bool = True
...
class PartnerSettingsDto(TrackableDto):
    ...
    yoomoney_commission: Decimal = Decimal("3.5")  # Комиссия YooMoney
    ...
    tbank_commission: Decimal = Decimal("2.8")
    cryptomus_commission: Decimal = Decimal("1.0")
    robokassa_commission: Decimal = Decimal("3.5")
    stripe_commission: Decimal = Decimal("3.5")
    mulenpay_commission: Decimal = Decimal("3.5")
    cloudpayments_commission: Decimal = Decimal("3.5")
    ...
    def get_gateway_commission(self, gateway_type: str) -> Decimal:
        ...
        commissions = {
            ...
            "YOOMONEY": self.yoomoney_commission,
            "TELEGRAM_STARS": self.telegram_stars_commission,
            "CRYPTOPAY": self.cryptopay_commission,
            "CRYPTOMUS": self.cryptomus_commission,
            "HELEKET": self.heleket_commission,
            "TBANK": self.tbank_commission,
            "ROBOKASSA": self.robokassa_commission,
            "STRIPE": self.stripe_commission,
            "MULENPAY": self.mulenpay_commission,
            "CLOUDPAYMENTS": self.cloudpayments_commission,
...
class LocalizedTextDto(TrackableDto):
    ru: str = ""
    en: str = ""

    @field_validator("ru", "en", mode="before")
    @classmethod
    def _normalize_locale_text(cls, value: object) -> str:
        return normalize_text(value)


class BrandingVerificationDto(TrackableDto):
    telegram_template: LocalizedTextDto = Field(
        default_factory=lambda: LocalizedTextDto(
            ru=(
                "{project_name} код верификации\n"
                "Код: {code}\n\n"
            ...
            en=(
                ...
                "Code: {code}\n\n"
                "Enter this code in your web profile to link Telegram."
            ),
        )
    )
    password_reset_telegram_template: LocalizedTextDto = Field(
        default_factory=lambda: LocalizedTextDto(
            ru=(
                "Код сброса пароля {project_name}\n"
                "Код: {code}\n\n"
                "Если это были не вы, просто проигнорируйте это сообщение."
            ),
            en=(
                "Your {project_name} password reset code:\n"
                "{code}\n\n"
                "If you did not request this, ignore this message."
            ),
        )
    )
    web_request_delivered: LocalizedTextDto = Field(
        default_factory=lambda: LocalizedTextDto(
            ru="Код верификации отправлен в Telegram",
            en="Verification code sent to Telegram",
        )
    )
    web_request_open_bot: LocalizedTextDto = Field(
        default_factory=lambda: LocalizedTextDto(
            ru="Код создан. Откройте чат с ботом, нажмите /start и повторите попытку",
            en="Code generated. Open bot chat, press /start and retry",
        )
    )
    web_confirm_success: LocalizedTextDto = Field(
        default_factory=lambda: LocalizedTextDto(
            ru="Telegram аккаунт успешно привязан",
            en="Telegram account linked successfully",
        )
    )

    @model_validator(mode="after")
    def _validate_templates(self) -> "BrandingVerificationDto":
        self.telegram_template.en = validate_template(
            self.telegram_template.en,
            field_name="verification.telegram_template.en",
            allowed_placeholders=("project_name", "code"),
            required_placeholders=("code",),
        )
        self.password_reset_telegram_template.en = validate_template(
            self.password_reset_telegram_template.en,
            field_name="verification.password_reset_telegram_template.en",
            allowed_placeholders=("project_name", "code"),
            required_placeholders=("code",),
        )
        self.web_request_delivered.en = validate_template(
            self.web_request_delivered.en,
            field_name="verification.web_request_delivered.en",
            allowed_placeholders=("project_name",),
        )
        self.web_request_open_bot.en = validate_template(
            self.web_request_open_bot.en,
            field_name="verification.web_request_open_bot.en",
            allowed_placeholders=("project_name",),
        )
        self.web_confirm_success.en = validate_template(
            self.web_confirm_success.en,
            field_name="verification.web_confirm_success.en",
            allowed_placeholders=("project_name",),
        )

        if normalize_text(self.telegram_template.ru):
            self.telegram_template.ru = validate_template(
                self.telegram_template.ru,
        ...
        if normalize_text(self.password_reset_telegram_template.ru):
            self.password_reset_telegram_template.ru = validate_template(
                self.password_reset_telegram_template.ru,
                field_name="verification.password_reset_telegram_template.ru",
                allowed_placeholders=("project_name", "code"),
                required_placeholders=("code",),
            )
        else:
        ...
        if normalize_text(self.web_request_delivered.ru):
            self.web_request_delivered.ru = validate_template(
                self.web_request_delivered.ru,
                field_name="verification.web_request_delivered.ru",
                allowed_placeholders=("project_name",),
            )
        else:
            self.web_request_delivered.ru = ""

        if normalize_text(self.web_request_open_bot.ru):
            self.web_request_open_bot.ru = validate_template(
                self.web_request_open_bot.ru,
                field_name="verification.web_request_open_bot.ru",
                allowed_placeholders=("project_name",),
            )
        else:
            self.web_request_open_bot.ru = ""

        if normalize_text(self.web_confirm_success.ru):
            self.web_confirm_success.ru = validate_template(
                self.web_confirm_success.ru,
                field_name="verification.web_confirm_success.ru",
                allowed_placeholders=("project_name",),
            )
        else:
            self.web_confirm_success.ru = ""
...
class BrandingSettingsDto(TrackableDto):
    ...
    def _normalize_project_name(cls, value: object) -> str:
        return ensure_text_length(
            ...
            field_name="branding.project_name",
            min_length=2,
            max_length=40,
        )

    @field_validator("web_title", mode="before")
    @classmethod
    def _normalize_web_title(cls, value: object) -> str:
        return ensure_text_length(
            normalize_text(value),
