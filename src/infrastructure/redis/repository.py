...
class RedisRepository:
    ...
    async def get(
        ...
        return cast(T, TypeAdapter(validator).validate_python(value))
    ...
    async def delete_pattern(self, pattern: str, *, count: int = 100) -> int:
        deleted_total = 0
        batch: list[Any] = []
        ...
        async for key in self.client.scan_iter(match=pattern, count=count):
            ...
            deleted_total += await cast(Awaitable[int], self.client.delete(*batch))
            batch.clear()
